...
exports.recycleDocumentNumber = recycleDocumentNumber;
exports.getNextRecycledNumber = getNextRecycledNumber;
exports.createDocument = createDocument;
exports.confirmDocument = confirmDocument;
exports.smartEditDocument = smartEditDocument;
...
function generateDocumentNumber(docType, customSeq) {
    ...
    const tx = db.transaction(() => {
        ...
        let row;
        try {
            row = db.prepare('SELECT last_seq, recycled_seqs FROM document_sequences WHERE doc_type = ? AND year = ?').get(docType, year);
        }
        catch {
            row = db.prepare('SELECT last_seq FROM document_sequences WHERE doc_type = ? AND year = ?').get(docType, year);
        }
        const lastSeq = row?.last_seq ?? 0;
        let recycled = [];
        try {
            recycled = JSON.parse(row?.recycled_seqs ?? '[]');
        }
        catch {
            recycled = [];
        }
...
function recycleDocumentNumber(docType, number) {
    const db = (0, connection_1.getDb)();
    const year = new Date().getFullYear() % 100;
    const prefix = DOC_PREFIXES[docType] ?? 'DOC';
    ...
    if (!match)
        return; // ليس رقماً حقيقياً (BRO- مثلاً)
...
function getNextRecycledNumber(docType) {
    const db = (0, connection_1.getDb)();
    const year = new Date().getFullYear() % 100;
    const prefix = DOC_PREFIXES[docType] ?? 'DOC';
    let row;
    try {
        row = db.prepare('SELECT recycled_seqs FROM document_sequences WHERE doc_type = ? AND year = ?').get(docType, year);
    }
    catch {
        return null;
    }
      ...
      `).get(id);
        ...
        `).get(id);
                if (link?.parent_id) {
                    ...
                    const cancelledDoc = db.prepare('SELECT number, type FROM documents WHERE id = ?').get(link.parent_id);
                    if (cancelledDoc) {
                        (0, accounting_service_1.deleteAccountingEntriesForCancelledDocument)(db, cancelledDoc.type, link.parent_id, cancelledDoc.number, userId);
                    }
                }
            }
 ...
 */
function smartEditDocument(docId, userId) {
    const db = (0, connection_1.getDb)();
    const doc = db.prepare('SELECT * FROM documents WHERE id = ? AND is_deleted = 0').get(docId);
    if (!doc)
        throw new Error('Document introuvable');
    if (doc.status !== 'confirmed')
        throw new Error('Seuls les documents confirmés peuvent être modifiés via Smart Edit');
  ...
  `).get(docId);
    ...
    const tx = db.transaction(() => {
        ...
        db.prepare('INSERT INTO document_links (parent_id, child_id, link_type) VALUES (?, ?, ?)').run(docId, avoirResult.id, 'smart_edit_avoir');
        ...
        (0, accounting_service_1.deleteAccountingEntriesForCancelledDocument)(db, doc.type, docId, doc.number, userId);
        (0, audit_service_1.logAudit)(db, {
            user_id: userId,
            action: 'SMART_EDIT_CANCEL',
            table_name: 'documents',
            record_id: docId,
        ...
        db.prepare('INSERT INTO document_links (parent_id, child_id, link_type) VALUES (?, ?, ?)').run(docId, newDocResult.id, 'smart_edit_replacement');
        (0, audit_service_1.logAudit)(db, {
            user_id: userId,
            action: 'SMART_EDIT_CREATE',
            table_name: 'documents',
 ...
 */
function getSubTableData(db, type, docId) {
    switch (type) {
        case 'invoice': {
            const row = db.prepare('SELECT * FROM doc_invoices WHERE document_id = ?').get(docId);
            return row ? {
        ...
        case 'quote': {
            const row = db.prepare('SELECT * FROM doc_quotes WHERE document_id = ?').get(docId);
            return row ? {
        ...
        case 'bl': {
            const row = db.prepare('SELECT * FROM doc_bons_livraison WHERE document_id = ?').get(docId);
            return row ? {
        ...
        case 'proforma': {
            const row = db.prepare('SELECT * FROM doc_proformas WHERE document_id = ?').get(docId);
            return row ? {
                validity_date: row.validity_date,
                currency: row.currency,
                exchange_rate: row.exchange_rate,
        ...
        case 'avoir': {
            const row = db.prepare('SELECT * FROM doc_avoirs WHERE document_id = ?').get(docId);
            return row ? {
        ...
        case 'purchase_invoice': {
            const row = db.prepare('SELECT * FROM doc_purchase_invoices WHERE document_id = ?').get(docId);
            return row ? {
                payment_method: row.payment_method,
                due_date: row.due_date,
                global_discount: row.global_discount ?? 0, // ✅ FIX: إضافة global_discount
        ...
        case 'import_invoice': {
            const row = db.prepare('SELECT * FROM doc_import_invoices WHERE document_id = ?').get(docId);
            return row ? {
                currency: row.currency,
                exchange_rate: row.exchange_rate,
...
function insertSubTable(db, type, docId, extra) {
    switch (type) {
        ...
        VALUES (?, ?, ?, ?, ?, ?)`).run(docId, extra.currency ?? 'MAD', extra.exchange_rate ?? 1, extra.payment_method ?? null, extra.due_date ?? null, extra.global_discount ?? 0 // ✅ FIX: إضافة global_discount
            );
            break;
        case 'quote':
            db.prepare(`INSERT INTO doc_quotes (document_id, validity_date, probability, global_discount) VALUES (?, ?, ?, ?)`).run(docId, extra.validity_date ?? null, extra.probability ?? 50, extra.global_discount ?? 0 // ✅ FIX: إضافة global_discount
            );
            break;
        case 'bl':
            db.prepare(`INSERT INTO doc_bons_livraison (document_id, delivery_address, delivery_date, global_discount) VALUES (?, ?, ?, ?)`).run(docId, extra.delivery_address ?? null, extra.delivery_date ?? null, extra.global_discount ?? 0 // ✅ FIX: إضافة global_discount
            );
            break;
        case 'proforma':
            db.prepare(`INSERT INTO doc_proformas (document_id, validity_date, currency, exchange_rate, global_discount) VALUES (?, ?, ?, ?, ?)`).run(docId, extra.validity_date ?? null, extra.currency ?? 'MAD', extra.exchange_rate ?? 1, extra.global_discount ?? 0 // ✅ FIX: إضافة global_discount
            );
            break;
        case 'avoir':
            db.prepare(`INSERT INTO doc_avoirs (document_id, avoir_type, affects_stock, reason, global_discount) VALUES (?, ?, ?, ?, ?)`).run(docId, extra.avoir_type ?? 'commercial', extra.affects_stock ? 1 : 0, extra.reason ?? null, extra.global_discount ?? 0 // ✅ FIX: إضافة global_discount
            );
            break;
        case 'purchase_invoice':
            db.prepare(`INSERT INTO doc_purchase_invoices (document_id, payment_method, due_date, global_discount) VALUES (?, ?, ?, ?)`).run(docId, extra.payment_method ?? null, extra.due_date ?? null, extra.global_discount ?? 0 // ✅ FIX: إضافة global_discount
            );
            break;
        case 'import_invoice':
            db.prepare(`
